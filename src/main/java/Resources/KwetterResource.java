...
public class KwetterResource {
    ...
    public Response create(KwetterJsonDTO k) {
        ...
        try {
            kwetter.setSelf(Link.fromUri(uriInfo.getBaseUri() + "Kwetters/" + kwetter.getId()).rel("self").type("GET").build());
            KwetterUser user = kwetter.getUser();
            user.setSelf(Link.fromUri(uriInfo.getBaseUri() + "/users/" + kwetter.getUser().getId()).rel("self").type("GET").build());
            kwetter.setUser(user);
            return Response.status(Response.Status.CREATED).entity(new ObjectMapper().writeValueAsString(kwetter)).links(Link.fromUri(uriInfo.getBaseUri() + "/Kwetters/").rel("self").type("DELETE").build()).build();
    ...
    public Response kwetterByID(@PathParam("id") int id) {
        ...
        if (kwetter != null) {
            try {
                kwetter.setSelf(Link.fromUri(uriInfo.getAbsolutePath()).rel("self").type("GET").build());
                KwetterUser user = kwetter.getUser();
                user.setSelf(Link.fromUri(uriInfo.getBaseUri() + "/users/" + kwetter.getUser().getId()).rel("self").type("GET").build());
                kwetter.setUser(user);
        ...
        if (kwetters != null) {
            try {
                for (Kwetter k : kwetters) {
                    k.setSelf(Link.fromUri(uriInfo.getBaseUri() + "Kwetters/" + k.getId()).rel("self").type("GET").build());
                    KwetterUser user = k.getUser();
                    user.setSelf(Link.fromUri(uriInfo.getBaseUri() + "/users/" + k.getUser().getId()).rel("self").type("GET").build());
                    k.setUser(user);
                }
